...
EOF
    ...
    MY_EKS_CLUSTER=`eksctl create cluster --name=${AWS_PROJECT}fargate --fargate` 
    aws eks tag-resource --resource-arn ${MY_EKS_CLUSTER} --tags <team>=<devs>
    #--tags Key=${TAGKEY0},Value=${TAGVALUE0} Key=${TAGKEY1},Value=${TAGVALUE1} Key=${TAGKEY2},Value=${TAGVALUE2}  #EKS cluster with Fargate profile
